...
<body>
    <div class="container">
        ...
        <div class="" style="background-color:olivedrab;padding: 5px 0;border-radius: 10px; margin-bottom: 20px;">
            <h3 style="margin-left: 20px;">Chi tiết sản phẩm</h3>
        </div>
        <?php
        $product = new products_class();
        if (isset($_GET['idchitiet'])) {
            ...
            $list_chi_tiet = $product->show_products_chi_tiet($id);
            if (isset($list_chi_tiet)) {
                while ($row = $list_chi_tiet->fetch_assoc()) {
        ?>
                    <div class="card mb-3" style="max-width: full;">
                        <div class="row g-0">
                            <div class="col-md-4">
                                <img src="../admin/upload/<?php echo $row['img_hanghoa']; ?>" class="img-fluid rounded-start" alt="...">
                            </div>
                            <div class="col-md-8">
                                <div class="card-body">
                                    <h5 class="card-title"><?php echo $row['name_hanghoa']; ?></h5>
                                    <h5 class="card-title">Giá :<?php echo $row['gia']; ?>VND Giảm <?php echo $row['giam_gia']; ?>%</h5>
                                    <p class="card-text"><?php echo $row['mo_ta']; ?></p>
                                    <p class="card-text"><small class="text-muted">Last updated <?php echo $row['ngay_tao']; ?></small></p>
                                    <?php
                                    $id_lh = $row['id_loaihang'];
                                    $id_hh = $row['id_hanghoa'];
                                    ?>
                                </div>
                            </div>
                            <hr>
                            <?php
                            // đếm lượt truy cập vào sp
                            if (session_id() == '') {
                                session_start();
                            }
                            if (!isset($_SESSION[$row['name_hanghoa']])) {
                                $_SESSION[$row['name_hanghoa']] = 1;
                            } else {
                                $_SESSION[$row['name_hanghoa']] += 1;
                            }
                            echo "Lượt truy cập : " . $_SESSION[$row['name_hanghoa']];
                            $luot_xem = $_SESSION[$row['name_hanghoa']];
                            $up_luot_xem = $product->up_luot_xem($luot_xem, $id);
                            ?>
                        </div>
                    </div>
                    <div class="" style="background-color:olivedrab;padding: 5px 0;border-radius: 10px; margin-bottom: 20px;">
                        <h4 style="margin-left:20px;color: white;">Sản phẩm liên quan</h4>
                    </div>
                    <div class="row row-cols-1 row-cols-md-5 g-4">
                        <!--  -->
                        <?php
                        $show_product_lq = $product->show_products_cung_loai($id_lh, $id_hh);
                    ...
                    <div class="full">
                        <div class="form-group">
                            <form action="" method="post">
                                ...
                                <textarea name="comment" style="border-radius: 15px; padding:10px 25px;"></textarea>
                                <button class="btn" name="btn_insert" style="border-radius: 10px;">Đăng bình luận</button>
                            </form>
                        </div>
                    </div>
        <?php
        ...
        ?>
        <hr>
        <label for="">Bình luận trước đó ...</label>
        <?php
        $user = new users_class();
        if (isset($_POST['btn_insert'])) {
            $date = date("Y/m/d");
            if (isset($_GET['idchitiet'])) {
                $id_hanghoa = $_GET['idchitiet'];
            }
            ...
            if (isset($id_hanghoa) && isset($date) && isset($noi_dung) && isset($id_khachhang)) {
                $up_comment = $user->add_binh_luan($noi_dung, $id_khachhang, $id_hanghoa, $date);
                if (isset($up_comment)) {
                    echo $up_comment;
                }
            }
        ...
        $user = new users_class();
        $id = $_GET['idchitiet'];
        $show = $user->show_binh_luan($id);
        if (isset($show)) {
            while ($row = $show->fetch_assoc()) {
        ?>
                <div class="user">
                    <img src="../admin/upload/<?php echo $row['img_khachhang']; ?>" style="width:40px; border-radius: 50%;">
                    <span style="font-size:large"><?php echo $row['name_taikhoan']; ?></span>
                </div>
                <div class="comment">
                    <p style="font-size:"><?php echo $row['noi_dung']; ?></p>
                </div>
        <?php
            }
        }
